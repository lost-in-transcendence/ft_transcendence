...
export function ChatSidebar({ user }: any) {
  ...
  if (ctx.ChatState.visibleChannels) {
    privMsgs = ctx.ChatState.visibleChannels.filter(
      (c) => c.mode === "PRIVMSG"
    );
  ...
  return (
    <div className="basis-0 flex">
      <div
        className="bg-gray-700 w-full h-screen rounded drop-shadow-lg
						md:w-52
...
function ChatModal(props: { isOpen: boolean; children: any; onClose: any }) {
  ...
  return ReactDOM.createPortal(
    <div
      className={`modal-overlay z-20 ${props.isOpen ? "modal-open" : ""}`}
      onClick={(e) => {
        e.stopPropagation();
        closeModal();
      }}
    >
      <div
        className="modal-content bg-gray-300 rounded"
        onClick={(e) => e.stopPropagation()}
      >
        <div className="modal-body">{props.isOpen && props.children}</div>
      ...
      <form className="flex flex-col" onSubmit={createChannel}>
        ...
        <label className="flex flex-row justify-between p-2">
          ...
          <select
            ...
            defaultValue={"PUBLIC"}
            onChange={(e) =>
              setData({ channelName: data.channelName, mode: e.target.value })
            }
          >
