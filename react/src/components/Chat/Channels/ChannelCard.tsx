...
import
{
	AiFillEyeInvisible as EyeClosed,
...
})
{
	...
	return (
		<div
			className={`${isActive && "bg-gray-500 rounded"} hover:bg-gray-500 hover:text-white text-left shadow`}
		>
			<span
				onClick={setActiveChannel}
				className={`${!joinable && "cursor-pointer"} flex-row items-center mx-3 text-xl overflow-hidden break-words inline-flex`}
			>
				{channel.mode === "PROTECTED" && joinable && (
					<LockedIcon className="mr-1" size={"17"} />
				)}
				{formatedName}
			</span>
			{joinable && (
...
function FormProtectedChannel({ onClose, channel }: any)
{
	...
	useEffect(() =>
	{
		context.ChatState.socket?.on("exception", (exception: any) =>
		{
			setUnauthorized("incorrect");
		});
		return () =>
		{
			context.ChatState.socket?.off("exception");
		};
	}, []);
	...
	{
		e.preventDefault();
		context.ChatState.socket?.emit(events.JOIN_CHANNEL, {
			...
			<label className="flex flex-row justify-center gap-4 p-2">
				...
				{passwordHidden ? (
					<EyeOpen
						...
						className="self-baseline relative -left-10 mt-0.5 text-gray-500 cursor-pointer"
					/>
				) : (
					<EyeClosed
						size={"20"}
